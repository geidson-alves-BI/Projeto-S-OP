...
_MISSING_SECTION_QUESTIONS: dict[str, str] = {
    "top_products": "Envie o ranking de top_products para identificar concentracao de prioridade por SKU.",
    "mts_products": "Envie a lista de mts_products para validar impacto dos itens make-to-stock.",
    "mto_products": "Envie a lista de mto_products para avaliar capacidade e lead time sob demanda.",
    "forecast_summary": "Envie forecast_summary para analisar tendencia, variacao e sinal de sazonalidade.",
...
class DeterministicProvider(BaseAIProvider):
    ...
    def generate(
        ...
        response: dict[str, Any] = {
            ...
            "data_quality_flags": [],
            "disclaimer": (
                "Insights gerados em modo deterministico com base exclusiva no context_pack. "
                "Nao ha inferencia fora dos dados enviados."
            ),
        }

        if validation.message:
            response["data_quality_flags"].append(validation.message)
        ...
        if not validation.is_valid:
            response["executive_summary"].append(
                "Dados insuficientes para gerar insights executivos confiaveis com o context_pack atual."
            )
            response["disclaimer"] = (
                "Dados insuficientes: complete o context_pack e regenere os insights antes de tomar decisoes."
            )
            return response
        ...
        if top_products:
            ...
            response["executive_summary"].append(summary_by_persona[persona.code])
        ...
        if forecast_summary:
            ...
            response["risks"].append(
                {
                    "title": forecast_risk_title[persona.code],
                    "severity": "medium",
            ...
            response["actions"].append(
                {
                    "title": forecast_action_title[persona.code],
                    "horizon": "0-7d",
                    "impact": "medium",
        ...
        if not response["executive_summary"]:
            response["executive_summary"].append(
                "Dados insuficientes para gerar leitura completa; confirme o preenchimento dos blocos do context_pack."
            )

        if not response["risks"]:
            response["risks"].append(
                {
                    "title": "Risco nao qualificado por ausencia de evidencias suficientes no context_pack.",
                    "severity": "low",
